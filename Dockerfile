###
# this layer uses the image iot-thin-edge-container-bundle and adds projects specific files. This is required to support PICS and EP5
...
# Copy container-image to /usr/bin and create symlink
COPY tedge/container-image /usr/bin/container-image
RUN chmod 755 /usr/bin/container-image && \
    ln -sf /usr/bin/container-image /etc/tedge/sm-plugins/container-image

# Copy manage-update.sh script
RUN mkdir -p /etc/tedge/scripts
